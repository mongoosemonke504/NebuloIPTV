...
struct SettingsView: View {
    ...
    var body: some View { 
        NavigationStack { 
            Form { 
                Section(header: Text("Appearance")) { Picker("Theme", selection: $appTheme) { ForEach(AppTheme.allCases) { t in Text(t.rawValue).tag(t.rawValue) } }.pickerStyle(.segmented); Picker("Layout Mode", selection: $viewMode) { ForEach(ViewMode.allCases, id: \.rawValue) { Text($0.rawValue).tag($0.rawValue) } }.pickerStyle(.segmented); ColorPicker("App Accent Color", selection: Binding(get: { Color(hex: customAccentHex) ?? .blue }, set: { if let h = $0.toHex() { customAccentHex = h } })) }; 
                NebulaSettingsSection(showSourceSelection: $showSourceSelection, showImagePicker: $showImagePicker, showFilePicker: $showFilePicker, inputImage: $inputImage); 
                Section(header: Text("Content Management")) {
                    ...
                    NavigationLink(destination: CategoriesManagerView(categories: $categories, accentColor: accentColor, viewModel: viewModel)) { Label("Manage Categories", systemImage: "list.bullet.rectangle.portrait.fill") }; NavigationLink(destination: HiddenChannelsSettingsView(viewModel: viewModel)) { HStack { Label("Hidden Channels", systemImage: "eye.slash.fill").foregroundColor(.red); Spacer(); if !viewModel.hiddenIDs.isEmpty { Text("\(viewModel.hiddenIDs.count)").foregroundStyle(.secondary) } } } }; Section(header: Text("Support")) { Toggle("Show Support Popup", isOn: $showSupportPopup); Link(destination: URL(string: "https://discord.gg/QkBUjsGCJ2")!) { Label("Join Discord", systemImage: "bubble.left.and.bubble.right.fill") }; Link(destination: URL(string: "https://buymeacoffee.com/mongoosemonke")!) { Label("Buy Me a Coffee", systemImage: "cup.and.saucer.fill") } }; Section { Button("Logout", role: .destructive) { withAnimation { viewModel.reset(); isLoggedIn = false } } } 
            }
        ...
        Section(header: Text("Background Settings")) {
            ...
            if useCustomBackground {
                Button(action: { showSourceSelection = true }) {
                    Label("Select Background Photo", systemImage: "photo.on.rectangle")
                }
