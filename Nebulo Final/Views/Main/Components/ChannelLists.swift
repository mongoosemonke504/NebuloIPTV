            ...
            ScrollView(.horizontal, showsIndicators: false) {
                LazyHStack(spacing: 16) {
                    ForEach(channels) { c in
                        ...
                        .opacity(viewModel.draggingChannel?.id == c.id ? 0.01 : 1.0)
                        .onDrag {
                            viewModel.draggingChannel = c
                            return NSItemProvider(object: String(c.id) as NSString)
                        } preview: {
                            SearchChannelContent(channel: c, viewModel: viewModel)
                                .frame(width: 180, height: 101)
                                .contentShape(Rectangle())
                        }
                        .onDrop(of: ["public.text"], delegate: ChannelDropDelegate(item: c, viewModel: viewModel, proxy: proxy))
                    }
                }
                .padding(.horizontal)
            }
            .onDrop(of: ["public.text"], delegate: ScrollDropDelegate(viewModel: viewModel))
        }
        .frame(height: 170)
...
struct ScrollDropDelegate: DropDelegate {
    ...
    func performDrop(info: DropInfo) -> Bool {
        viewModel.commitChannelOrder()
        viewModel.draggingChannel = nil
        return true
    }
}

struct ChannelDropDelegate: DropDelegate {
    let item: StreamChannel
    let viewModel: ChannelViewModel
    let proxy: ScrollViewProxy?
    
    func dropEntered(info: DropInfo) {
        guard let dragging = viewModel.draggingChannel, dragging.id != item.id else { return }
        viewModel.moveChannelInSearch(from: dragging, to: item, save: false)
