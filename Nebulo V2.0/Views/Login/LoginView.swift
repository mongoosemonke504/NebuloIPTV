...
struct LoginView: View {
    ...
    var body: some View {
        NavigationStack {
                        ...
                        VStack(spacing: 24) {
                            ...
                            VStack(spacing: 16) {
                                ...
                                if selectedLoginType == .xtream {
                                    ...
                                    GlassTextField(icon: "cpu", placeholder: "00:1A:79...", text: $macInput)
                                        .onChange(of: macInput) { nv in formatMAC(nv) }
                                }
                            }
            ...
            .onChange(of: urlInput) { nv in if selectedLoginType == .xtream { parseM3ULink(nv) } }
            .onAppear {
    ...
    func login() { let cl = urlInput.trimmingCharacters(in: .whitespaces); var safe = cl; if safe.hasSuffix("/") { safe = String(safe.dropLast()) }; if selectedLoginType == .xtream { guard !usernameInput.isEmpty, !passwordInput.isEmpty, !safe.isEmpty else { errorMessage = "Please enter your server URL, username, and password."; showError = true; return } } else if selectedLoginType == .mac { guard !macInput.isEmpty, macInput.count >= 17, !safe.isEmpty else { errorMessage = "Please enter a valid Portal URL and MAC Address."; showError = true; return } } else { guard !safe.isEmpty else { errorMessage = "Please enter a valid Playlist URL."; showError = true; return } }; xstreamURL = safe; username = usernameInput; password = passwordInput; macAddress = macInput; loginTypeRaw = selectedLoginType.rawValue; withAnimation(.easeInOut(duration: 0.5)) { isLoggedIn = true } }
}
