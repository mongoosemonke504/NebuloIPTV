...
struct GridVideoPlayer: UIViewControllerRepresentable {
    ...
    func makeUIViewController(context: Context) -> AVPlayerViewController { let c = AVPlayerViewController(); let h: [String: Any] = ["User-Agent": "Mozilla/5.0 (iPhone; CPU iPhone OS 16_0 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/16.0 Mobile/15E148 Safari/604.1"]; let a = AVURLAsset(url: url, options: ["AVURLAssetHTTPHeaderFieldsKey": h]); let i = AVPlayerItem(asset: a); i.preferredForwardBufferDuration = 0; let p = AVPlayer(playerItem: i); p.isMuted = isMuted; c.player = p; c.showsPlaybackControls = false; c.videoGravity = .resizeAspect; p.play(); return c }
    func updateUIViewController(_ ui: AVPlayerViewController, context: Context) { ui.player?.isMuted = isMuted }
}
